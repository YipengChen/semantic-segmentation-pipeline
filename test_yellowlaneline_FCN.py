...
model_2 = torch.load('./checkpoints/model_250.pt')
model_1.eval()
model_1 = model_1.to(device)
model_2.eval()
model_2 = model_2.to(device)
torch.save(model_2.state_dict(), 'checkpoints/yellow_laneline_seg_model_FCNs_s_v2_e250_202101201728.pth')
image_transforms = transforms.Compose([transforms.ToTensor()])

test_image_path = '/home/chenyp/dataset/laneline_3'
...
with torch.no_grad():
    for image_name in images_list:
        ...
        output_2 = model_2(image_t)
        print(output_2.shape)
        s_t = time.time()
        output_2 = output_2.cpu().numpy()
        print('cpu() time:{}'.format(time.time()-s_t))
        output_2 = output_2[0]
        print(output_2.shape)
